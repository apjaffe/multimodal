...
class EncoderDecoder:
  def __init__(self, model, imgs, captions_src, model_file, token_file, min_freq, embed_size, hidden_size, image_size, dropout, builder):
    self.model = model
  ...
  def make_caption(self, img, max_len = 50):
    ...
    while len(trans_sentence) < max_len:
        ...
        c_t = dy.concatenate([img_vec, embed_t])
        dec_state = dec_state.add_input(c_t)
  ...
  def step_batch(self, batch, cnum):
    ...
    img_vec = dy.reshape(concat, (self.image_size,), batch_size = num_batches)
